...
class MGPooling(nn.Module):
    def __init__(self, nchannels, reduction = 16):
        ...
        self.avg_conv = nn.Conv2d(in_channels=self.split_channels[0],out_channels= self.split_channels[0],
                                         kernel_size= 1, stride=1, padding=0, bias = False)
        self.max_conv = nn.Conv2d(in_channels=self.split_channels[1],out_channels=self.split_channels[1],
                                         kernel_size= 1, stride=1, padding=0, bias = False)
        ...
        self.fc = nn.Sequential(
            nn.Linear(nchannels*5, nchannels // reduction, bias=False),
    ...
    def forward(self, x):
        ...
        avg_conv = self.avg_conv(split_x[0])
        max_conv = self.max_conv(split_x[1])                                            
        
        channel_1 = self.split_channels[0]
        channel_2 = self.split_channels[1]
        # different pooling methods 
        avg_pool = self.avg_pool(avg_conv).view(b, channel_1)
        avg_pool2 = self.avg_pool2(avg_conv).view(b, 4*channel_1)
        #avg_pool4 = self.avg_pool4(x)
        
      
        max_pool = self.max_pool(max_conv).view(b, channel_2)
        max_pool2 = self.max_pool2(max_conv).view(b, 4*channel_2)
